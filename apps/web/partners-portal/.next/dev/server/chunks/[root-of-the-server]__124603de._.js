...
"[project]/apps/web/partners-portal/src/lib/redis/tutor.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function getLearningProfile(userId) {
    const data = await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].hgetall(`learning_profile:${userId}`);
    if (!data || !('userId' in data)) return null;
    const profile = {
        ...data
    };
    if (typeof profile.completedCourses === 'string') {
        profile.completedCourses = JSON.parse(profile.completedCourses || '[]');
    }
    if (typeof profile.strugglingTopics === 'string') {
        profile.strugglingTopics = JSON.parse(profile.strugglingTopics || '[]');
    }
    if (typeof profile.strongTopics === 'string') {
        profile.strongTopics = JSON.parse(profile.strongTopics || '[]');
    }
    if (typeof profile.questionsAsked === 'string') {
        profile.questionsAsked = JSON.parse(profile.questionsAsked || '[]');
    }
    if (typeof profile.recommendedNextTopics === 'string') {
        profile.recommendedNextTopics = JSON.parse(profile.recommendedNextTopics || '[]');
    }
    return profile;
}
async function createLearningProfile(userId) {
    ...
    await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].hset(`learning_profile:${userId}`, {
        userId,
        completedCourses: JSON.stringify(profile.completedCourses),
        skillLevel: profile.skillLevel,
        preferredPace: profile.preferredPace,
        learningStyle: profile.learningStyle,
        strugglingTopics: JSON.stringify(profile.strugglingTopics),
        strongTopics: JSON.stringify(profile.strongTopics),
        questionsAsked: JSON.stringify(profile.questionsAsked),
        recommendedNextTopics: JSON.stringify(profile.recommendedNextTopics),
        conversationCount: 0,
        lastActiveAt: now
    });
    return profile;
}
async function updateLearningProfile(userId, updates) {
    const profile = await getLearningProfile(userId) || await createLearningProfile(userId);
    const updated = {
        ...profile,
        ...updates,
    ...
    if (Array.isArray(data.completedCourses)) {
        data.completedCourses = JSON.stringify(data.completedCourses);
    }
    if (Array.isArray(data.strugglingTopics)) {
        data.strugglingTopics = JSON.stringify(data.strugglingTopics);
    }
    if (Array.isArray(data.strongTopics)) {
        data.strongTopics = JSON.stringify(data.strongTopics);
    }
    if (Array.isArray(data.questionsAsked)) {
        data.questionsAsked = JSON.stringify(data.questionsAsked);
    }
    if (Array.isArray(data.recommendedNextTopics)) {
        data.recommendedNextTopics = JSON.stringify(data.recommendedNextTopics);
    }
    await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].hset(`learning_profile:${userId}`, data);
}
async function getTutorConversation(conversationId) {
    const data = await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].hgetall(`tutor_conv:${conversationId}`);
    ...
    if (typeof conv.learningProfile === 'string') {
        conv.learningProfile = JSON.parse(conv.learningProfile);
    }
    if (typeof conv.isActive === 'string') {
        conv.isActive = conv.isActive === '1' || conv.isActive === 'true';
    }
    return conv;
}
async function createTutorConversation(userId, courseId) {
    const convId = `tutor_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const now = new Date().toISOString();
    const profile = await getLearningProfile(userId) || await createLearningProfile(userId);
    const conversation = {
        id: convId,
    ...
    await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].hset(`tutor_conv:${convId}`, {
        id: convId,
        userId,
        courseId: courseId || '',
        messages: JSON.stringify([]),
        learningProfile: JSON.stringify(profile),
        createdAt: now,
        updatedAt: now,
        isActive: 1
    });
    await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].sadd(`user:${userId}:tutor_convs`, convId);
    return conversation;
}
async function addMessageToConversation(conversationId, message) {
    const conversation = await getTutorConversation(conversationId);
    if (!conversation) throw new Error('Conversation not found');
    conversation.messages.push(message);
    conversation.updatedAt = new Date().toISOString();
    await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].hset(`tutor_conv:${conversationId}`, {
        messages: JSON.stringify(conversation.messages),
        updatedAt: conversation.updatedAt
    });
}
async function getUserConversations(userId, limit = 10) {
    const convIds = await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].smembers(`user:${userId}:tutor_convs`);
    if (!convIds.length) return [];
    const conversations = await Promise.all(convIds.map((id)=>getTutorConversation(id)));
    return conversations.filter((c)=>c !== null).sort((a, b)=>new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).slice(0, limit);
}
async function createRecommendation(userId, recommendation) {
    const recId = `rec_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const data = {
        ...recommendation,
        id: recId
    };
    await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].hset(`recommendation:${recId}`, data);
    await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].sadd(`user:${userId}:recommendations`, recId);
    await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].zadd(`recommendations:active`, {
        score: new Date(recommendation.suggestedAt).getTime(),
        member: recId
    });
}
async function getUserRecommendations(userId) {
    const recIds = await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].smembers(`user:${userId}:recommendations`);
    if (!recIds.length) return [];
    const recommendations = await Promise.all(recIds.map(async (id)=>{
        const data = await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].hgetall(`recommendation:${id}`);
        return data;
    }));
...
"[project]/apps/web/partners-portal/src/app/api/partners/tutor/profile/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$auth$2f$session$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/apps/web/partners-portal/src/lib/auth/session.ts [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$tutor$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/apps/web/partners-portal/src/lib/redis/tutor.ts [app-route] (ecmascript)");
;
;
;
async function GET(request) {
    try {
        const { user } = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$auth$2f$session$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireSession"])();
        let profile = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$tutor$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getLearningProfile"])(user.id);
        if (!profile) {
            profile = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$tutor$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createLearningProfile"])(user.id);
        }
        const conversations = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$tutor$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getUserConversations"])(user.id, 5);
        const recommendations = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$tutor$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getUserRecommendations"])(user.id);
        const analytics = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$tutor$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getTutorAnalytics"])(user.id);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$29$2e$0_react$2d$dom$40$19$2e$2$2e$3_react$40$19$2e$2$2e$3_$5f$react$40$19$2e$2$2e$3$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            profile,
            conversations,
            recommendations,
...
async function PATCH(request) {
    try {
        ...
        const { skillLevel, preferredPace, learningStyle } = body;
        const updates = {};
        if (skillLevel) updates.skillLevel = skillLevel;
        if (preferredPace) updates.preferredPace = preferredPace;
        if (learningStyle) updates.learningStyle = learningStyle;
        await (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$tutor$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["updateLearningProfile"])(user.id, updates);
        const profile = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$partners$2d$portal$2f$src$2f$lib$2f$redis$2f$tutor$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getLearningProfile"])(user.id);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$29$2e$0_react$2d$dom$40$19$2e$2$2e$3_react$40$19$2e$2$2e$3_$5f$react$40$19$2e$2$2e$3$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            profile
        });
    } catch (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__124603de._.js.map