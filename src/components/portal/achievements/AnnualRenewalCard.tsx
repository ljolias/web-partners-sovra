    ...
    <Card className="p-6 bg-dark-surface border border-white/5 card-hover-gradient">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-neutral">{t('rewards.renewal_date')}</p>
            <p className="font-semibold text-white">
              {renewalDate.toLocaleDateString()}
            </p>
          </div>
          <div>
            <p className="text-neutral">{t('rewards.days_until_renewal')}</p>
            <p className={`font-semibold ${status.daysUntilRenewal <= 30 ? 'text-amber-400' : 'text-white'}`}>
